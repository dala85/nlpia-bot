...
LOGLEVEL = getattr(env, 'loglevel', DEFAULT_CONFIG.get(
...
def parse_args(args):
    ...
    parser.add_argument(
        '-b',
        '--bots',
        # DEFAULT_CONFIG['bots'],  # None so config.ini can populate defaults
        default=None,
        dest="bots",
...
def setup_logging(loglevel=LOGLEVEL):
    ...
    logging.basicConfig(level=loglevel, stream=sys.stdout,
                        format=logformat, datefmt=logdatefmt)
...
def parse_argv(argv=sys.argv):
    ...
    if len(argv) > 1:
        ...
        log.error(
            f"It doesn't look like you're running this as a command line application. sys.argv: {argv}")
    ...
    loglevel = min(args.loglevel or logging.WARNING,
                   args.verbosity or logging.WARNING)
    ...
    args.bots = 'glossary,qa' if getattr(
        args, 'bots', None) is None else args.bots
...
try:
    ...
    log.error(e)
    log.error(
        'Unable to parse command line arguments. Are you trying to import this into gunicorn?')
...
LANG = getattr(args, 'spacy_lang', None) or DEFAULT_CONFIG.get(
    'spacy_lang') or LANGS[0]
...
# clibot.py
# 'search_fuzzy', 'parul', 'eliza', 'glossary', 'qa'
DEFAULT_BOTS = ['pattern']
...
class passthroughSpaCyPipe:
    ...
    def __call__(self, doc):
        log.info(
            f"This passthroughSpaCyPipe component only logs the token count: {len(doc)}")
        return doc
